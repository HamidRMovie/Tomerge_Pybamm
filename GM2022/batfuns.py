...
def cycle_adaptive_simulation(model, parameter_values, experiment,SOC_0=1, save_at_cycles=None, drive_cycle=None):
    experiment_one_cycle = pybamm.Experiment(
        ...
        cccv_handling=experiment.cccv_handling,
                drive_cycles={"US06_GM": drive_cycle})
    Vmin = 2.7
    Vmax = 4.2
...
def plotc(all_sumvars_dict,esoh_data):
    esoh_vars = ["x_100", "y_0", "C_n", "C_p", "Capacity [A.h]", "Loss of lithium inventory [%]","x_0","y_100"]
...
def load_data_calendar(cell,eSOH_DIR,oCV_DIR):
    param = pybamm.LithiumIonParameters() 
    cell_no = f'{cell:02d}'
    dfe=pd.read_csv(eSOH_DIR+"aging_param_cell_IRC_cell0"+cell_no+".csv")
    
#     dfo=pd.read_csv(oCV_DIR+"ocv_data_cell_"+cell_no+".csv") #removed to check
    dfo=0
...
#         SOC_0 = 0.9
#         Temp = 45
    if cell_no=='93'or cell_no=='34':
        SOC_0=1.00
        Temp=25
        
    elif cell_no=='87' or cell_no=='28':
        SOC_0=.80
        Temp=25 
    elif cell_no=='56' or cell_no=='4':
        SOC_0=.50
        Temp=25
        
    elif cell_no=='57' or cell_no=='19':
        SOC_0=.80
        Temp=45
    elif cell_no=='94' or cell_no=='39':
        SOC_0=.50
        Temp=45
    eps_n_data = parameter_values.evaluate(C_n_init*3600/(param.n.L * param.n.prim.c_max * param.F* param.A_cc))
    eps_p_data = parameter_values.evaluate(C_p_init*3600/(param.p.L * param.p.prim.c_max * param.F* param.A_cc))        
...
def load_data(cell,eSOH_DIR,oCV_DIR): 
    ...
    cell_no = f'{cell:02d}'
    dfe=pd.read_csv(eSOH_DIR+"aging_param_cell_IRC_cell0"+cell_no+".csv")
    dfe_0=pd.read_csv(eSOH_DIR+"aging_param_cell_IRC_cell0"+cell_no+".csv")
